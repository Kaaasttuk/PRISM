...
class Experiment:
    def __init__(self, directory, extracted_dir, custom_filename_splitter=None, custom_img_path_generator = None, save_filetype = None, mean_amount = None):
        self.directory = directory
        if extracted_dir is not None:
            self.extracted_dir = extracted_dir
        else:
            self.extracted_dir = os.path.join(directory, "extracted")
        self.files = glob(os.path.join(self.extracted_dir, "*"))
    ...
    def trench_path_generator(self, directory, FOV, channel, trench, time, z=None, file_extension=None):
        # Use the default file extension if not provided.
        if file_extension is None:
            file_extension = self.save_file_extension
        # Convert coordinates to string values.
        FOV, channel, time = self.coordinate_converter(FOV, channel, time)
        # If a z value is given, include it.
        if z is not None:
            return "{}/{}_{}_TR{}_Z{:02d}_{}.{}".format(directory, FOV, channel, str(trench).zfill(2), z, time, file_extension)
        else:
    ...
    def mean_t_x(self, FOV, channel, sigma=False, x_crop = None, y_crop = None, *, plot=False):            
        mean_img = self.get_mean_of_timestack(FOV, channel)
        if y_crop is not None:
            mean_img = mean_img[y_crop[0]:y_crop[1],:]
    ...
    def find_trench_peaks(self, FOV, channel=None, sigma=False, distance=40, height=0, prominence=0, threshold=0, conv_filter = None, x_crop = None, y_crop = None, *, plot=False):
        if channel is not None:
            pass
        else:
            channel = self._registration_channel
        mean_img = self.mean_t_x(FOV, channel, sigma, x_crop, y_crop)
    ...
    def find_lane_peaks(self, FOV, channel=None, sigma=False, distance=1, height=1, *, plot=False, y_crop=[0, -1]):
        if channel is not None:
            pass
        else:
            channel = self._registration_channel
        mean_img = self.mean_t_y(FOV, channel, sigma)
        peaks, _ = find_peaks(mean_img[y_crop[0]:y_crop[1]], distance=distance, height=height)
        peaks = peaks + y_crop[0]
        if plot:
            plt.plot(mean_img)
            plt.title(f"FOV: {FOV}, Channel: {channel}, Gaussian sigma: {sigma}")
            plt.plot(peaks, mean_img[peaks], "x", c="r")
    ...
    def find_all_trench_y_positions_PC(self, channel=None, *, sigma=False, distance=1, height=1, plot=False, plot_save = False, y_crop=[0, -1], fixed_peak=None):
        ...
        self.y_peaks = {
            FOV: self.find_lane_peaks(FOV, channel=channel, sigma=sigma, distance=distance, height=height, plot=False, y_crop=y_crop)[
                1] for FOV in self.FOVs}
        if fixed_peak:
            self.y_peaks = {
            FOV: np.array([fixed_peak]) for FOV in self.FOVs}
            # self.y_peaks['xy049'] = np.array([1000])
            # self.y_peaks['xy051'] = np.array([1000])
            # self.y_peaks['xy023'] = np.array([1000])
        if plot:
            subplots = self.num_FOVs
            cols = 2
            rows = round(np.ceil(subplots / cols))
    ...
    #     trenches = []
    #     print(f"x_pos for FOV {FOV}: {x_pos}")
    ...
    # def get_trenches(self, FOV, channel, time, save=False):
    #     # Get the image based on the registration status
    #     image = self.get_image(FOV, channel, time, registered=self.is_registered)
        
    #     # Ensure y_peaks for the FOV is correctly retrieved
    #     if FOV not in self.y_peaks or len(self.y_peaks[FOV]) == 0:
    #         raise ValueError(f"y_peaks not found or empty for FOV {FOV}. Check your peak finding process.")
        
    #     # Retrieve y-position and x-position for trench extraction
    #     y_pos = self.y_peaks[FOV][0]  # Assuming single y-peak, modify if there are multiple
    #     x_pos = self.pruned_experiment_trench_x_lims[FOV]
        
    #     # Debugging output to verify x_pos contents
    #     print(f"x_pos for FOV {FOV}: {x_pos}")
        
    #     # Validate x_pos to ensure it is a dictionary of tuples
    #     if not isinstance(x_pos, dict):
    #         raise TypeError(f"x_pos for {FOV} should be a dictionary, got {type(x_pos)}")
        
    #     trenches = []
    #     for key, (L, R) in x_pos.items():
    #         if not (isinstance(L, int) and isinstance(R, int)):
    #             raise TypeError(f"Expected (L, R) to be integers, got {L}, {R}")
    #         trench = image[y_pos - self.trench_y_offsets[0]:y_pos - self.trench_y_offsets[1], L:R]
    #         trenches.append(trench)

    #     # Save each trench if requested
    #     if save:
    #         for i, trench in enumerate(trenches):
    #             out_path = self.trench_path_generator(self.trench_directory, FOV, channel, i, time, self.save_file_extension)
    #             self.imwriter(out_path, trench)
    ...
    #     # Create a list of all field combinations to be processed
    #     fields = product(self.FOVs, self.channels, self.times)
        
    #     # Attempt to create a directory for saving trenches
    #     try:
    ...
    #         print(f"Error during trench extraction: {e}")
            
    #         # If force=True, attempt to re-run the extraction process
    #         if force:
    ...
    # original
    # def get_trenches(self, FOV, channel, time, save=False):
    #     # Retrieve the image for the given FOV, channel, and time
    #     image = self.get_image(FOV, channel, time, registered=self.is_registered)
    #     # Get the y-position and x-limits
    #     y_pos = self.y_peaks[FOV][0]
    #     x_pos = self.pruned_experiment_trench_x_lims[FOV]
        
    #     # Debug print to check the content of x_pos
    #     print(f"x_pos for FOV {FOV}: {x_pos}")
        
    #     trenches = []
        
    #     # If x_pos is a dictionary, we should iterate over its values
    #     try:
    #         for i, (L, R) in enumerate(x_pos.values()):
    #             # Ensure L and R are integers
    #             L, R = int(L), int(R)
    #             # Define the trench area using y and x coordinates
    #             trench = image[y_pos - self.trench_y_offsets[0]:y_pos - self.trench_y_offsets[1], L:R]
    #             trenches.append(trench)
    #     except TypeError as e:
    #         print(f"Error unpacking x_pos for FOV {FOV}: {e}")
        
    #     # Save trenches if requested
    #     if save:
    #         for i, trench in enumerate(trenches):
    #             out_path = self.trench_path_generator(
    #                 self.trench_directory, FOV, channel, i, time, self.save_file_extension
    #             )
    #             self.imwriter(out_path, trench)

    def get_trenches(self, FOV, channel, time, z=None, save=False):
        # Retrieve the image for the given FOV, channel, and time
        image = self.get_image(FOV, channel, time, registered=self.is_registered)
        # Get the y–position and x–limits
        y_pos = self.y_peaks[FOV][0]
        ...
        # Debug: print the x limits
        print(f"x_pos for FOV {FOV}: {x_pos}")
        
        trenches = []
        # Iterate over each trench (assuming x_pos is a dictionary of (L, R) values)
        try:
            for i, (L, R) in enumerate(x_pos.values()):
                L, R = int(L), int(R)
                trench = image[y_pos - self.trench_y_offsets[0]:y_pos - self.trench_y_offsets[1], L:R]
        ...
        if save:
            for i, trench in enumerate(trenches):
                # Use the new generator passing the current z–slice.
                out_path = self.trench_path_generator(
                    self.trench_directory, FOV, channel, i, time, z, self.save_file_extension
                )
                self.imwriter(out_path, trench)
        return trenches


    def extract_trenches(self, n_jobs=-1, force=False, trenches_z="trenches_z00"):
        # Create a list of fields to process
        fields = product(self.FOVs, self.channels, self.times)
        try:
            # Create trenches directory if it doesn't exist
            os.mkdir(os.path.join(self.directory, trenches_z))
            # Use parallel processing to extract trenches for each field
            Parallel(n_jobs=n_jobs)(delayed(self.get_trenches)(*field, True) for field in fields)
        except:
            if force:
                warnings.warn("Re-extracting trenches")
    ...
    # def extract_all_trenches(self, n_jobs=-1, force=False):
    #     """
    #     For each FOV, timepoint, channel, and z–slice, extract the cropped trench images and
    #     save them all into one output folder (self.trench_directory) using a name of the form:
    #     {FOV}_TR{trench:02d}_{channel}_Z{z:02d}_TT{time}.{ext}
    #     """
    #     out_dir = self.trench_directory  # e.g. "D:/PRISM/trenches/"
    #     os.makedirs(out_dir, exist_ok=True)

    #     # Build a list of tasks over all FOVs, timepoints, channels and z–slices.
    #     tasks = []
    #     num_z = self.nd2_dimensions.get("Z", 1)
    #     for FOV, time, channel, z in product(self.FOVs, self.times, self.channels, range(num_z)):
    #         tasks.append((FOV, time, channel, z))
        
    #     # Use parallel processing to extract and save each trench.
    #     Parallel(n_jobs=n_jobs)(
    #         delayed(self._extract_trench_from_file)(FOV, time, channel, z, out_dir)
    #         for FOV, time, channel, z in tqdm(tasks, desc="Extracting trenches")
    #     )

    # def _extract_trench_from_file(self, FOV, time, channel, z, out_dir):
    #     """
    #     Given a specific FOV, time, channel and z–slice, load the corresponding image from
    #     the appropriate subfolder, then for every trench (as defined by the x–limits in 
    #     self.pruned_experiment_trench_x_lims[FOV]) crop the image (using the y–position in 
    #     self.y_peaks[FOV] and self.trench_y_offsets) and save it with a file name like:
        
    #         {FOV}_TR{trench:02d}_{channel}_Z{z:02d}_TT{time}.{ext}
        
    #     The cropped images are saved into out_dir.
    #     """
    #     # Construct the file path for the image from the extracted data.
    #     subfolder = os.path.join(self.extracted_dir, f"extracted_z{str(z).zfill(2)}")
    #     file_name = f"{FOV}_{channel}_{time}.{self.file_extension}"
    ...
    #     # Read the image.
    #     img = self.imreader(file_path)
        
    #     # Get the y–position (for cropping vertically)
    #     try:
    #         y_pos = self.y_peaks[FOV][0]
    #     except Exception as e:
    #         print(f"Error retrieving y–peak for FOV {FOV}: {e}")
    #         return
        
    #     # Get the trench x–limits for this FOV.
    #     if FOV not in self.pruned_experiment_trench_x_lims:
    #         print(f"No trench boundaries found for FOV {FOV}")
    #         return
    #     trench_bounds = self.pruned_experiment_trench_x_lims[FOV]
        
    #     # Loop over each trench and crop/save the image.
    #     for trench_id, (L, R) in trench_bounds.items():
    #         # Crop the image vertically using the preset offsets relative to y_pos and horizontally using L and R.
    #         trench_img = img[y_pos - self.trench_y_offsets[0] : y_pos - self.trench_y_offsets[1], L:R]
    #         # Build the output file name.
    #         out_file = os.path.join(
    #             out_dir,
    #             f"{FOV}_TR{str(trench_id).zfill(2)}_{channel}_Z{str(z).zfill(2)}_TT{time}.{self.save_file_extension}"
    #         )
    #         # Write the cropped trench image.
    #         self.imwriter(out_file, trench_img)
